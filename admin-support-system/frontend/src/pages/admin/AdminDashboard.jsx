...
export default function AdminDashboard() {
  ...
  const kpis = {
    ...
    online: engineers.filter(e => e.is_online).length,
  };
                                ...
                                <td className="px-4 py-3">
                                  {eng ? (
                                    <div className="flex items-center gap-1.5">
                                      <span className={`w-2 h-2 rounded-full shrink-0 ${eng.is_online ? 'bg-emerald-500' : 'bg-gray-400'}`} />
                  ...
                  {engineers.map(eng => {
                    ...
                    return (
                      <tr key={eng.support_id} className="border-b border-gray-100 dark:border-gray-800">
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-1.5">
                            {eng.is_online ? <Wifi size={14} className="text-emerald-500" /> : <WifiOff size={14} className="text-gray-400" />}
                            <span className={`text-xs font-medium ${eng.is_online ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-400'}`}>
                              {eng.is_online ? 'Online' : 'Offline'}
                            </span>
                          </div>
                        </td>
                        <td className="px-5 py-3.5">
