...
export default function ChatPanel({ ticket, onBack, readOnly = false, onStatusUpdate }) {
  ...
  // Issue 5: Track server-enforced read-only status
      ...
      ws.onmessage = (event) => {
        ...
        try {
          ...
          if (data.type === 'history') {
            ...
            // Issue 5: Server tells us if we're read-only (non-assigned engineer)
    ...
    <div className="flex flex-col flex-1 min-h-0 bg-gray-50 dark:bg-gray-950">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        ...
        {messages.map((msg, i) => {
          ...
          // Support engineer's own messages go to the RIGHT; customer messages go LEFT
          const isMe = !isSystem && msg.sender_type === 'support';
          // Support sees engineer name on their side, "Customer" (no real name) on customer side
          const label = isSystem
            ? 'System'
            : isMe
              ? (msg.sender_name || 'Support')
              : 'Customer'; // Never show customer real name to support

          if (isSystem) {
            return (
              <div key={msg.message_id || i} className="flex justify-center">
                <div className="max-w-[80%] bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 text-[11px] px-3 py-1.5 rounded-full text-center">
                  {msg.message}
                </div>
              </div>
            );
        ...
        {customerTyping && (
          <div className="flex justify-start">
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl rounded-bl-md px-4 py-2.5 flex items-center gap-2">
              <span className="flex gap-1">
                <span className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                <span className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                <span className="w-1.5 h-1.5 bg-gray-400 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
              </span>
              <span className="text-[10px] text-gray-400">Customer is typing</span>
            </div>
          </div>
        )}
        <div ref={bottomRef} />
      ...
      {/* Input — hidden for read-only viewers (Issue 5) */}
      ...
      {!isClosed && serverReadOnly && (
        <div className="bg-amber-50 dark:bg-amber-950/30 border-t border-amber-200 dark:border-amber-800 p-3 text-center text-xs text-amber-700 dark:text-amber-400">
          👁 Read-only — only the assigned engineer can send messages on this ticket.
        </div>
      )}
    </div>
  );
